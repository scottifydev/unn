...
create or replace function auth.has_role(required user_role)
returns boolean as $$
  select case required
    when 'reader' then true
    when 'contributor' then auth.user_role() in ('contributor', 'editor', 'admin')
    when 'editor' then auth.user_role() in ('editor', 'admin')
    when 'admin' then auth.user_role() = 'admin'
    else false
  end
$$ language sql security definer stable;
...
create policy "Admins can manage sections"
  on sections for all using (auth.has_role('admin'));
...
create policy "Editors can update any article"
  on articles for update using (auth.has_role('editor'));
...
create policy "Admins can delete any article"
  on articles for delete using (auth.has_role('admin'));
...
create policy "Editors can manage ticker items"
  on ticker_items for all using (auth.has_role('editor'));
...
create policy "Editors can manage advisories"
  on council_advisories for all using (auth.has_role('editor'));
...
create policy "Editors can manage ads"
  on ads for all using (auth.has_role('editor'));
...
create policy "Editors can manage trending"
  on trending for all using (auth.has_role('editor'));
